...
from fastapi import FastAPI, WebSocket, WebSocketDisconnect, UploadFile, File, Form, HTTPException, Query, BackgroundTasks
...
REQUIRED_DIRS = ["templates", "static/css", "static/js", "uploads", "exports", "logs"]
...
async def employee_management(request: Request):
    user = get_current_user(request)
    return templates.TemplateResponse("employees.html", {"request": request, "user": user})

@app.get("/detailed-attendance", tags=["Pages"])
async def detailed_attendance(request: Request):
    user = get_current_user(request)
...
async def startup():
    try:
        ...
        # 1. Load face embeddings from DB
        face_map = db_service.load_all_embeddings()

        # 2. Load FAISS (GPU) FIRST
        # This will claim the GPU context
        logger.info("Loading FAISS (GPU) Index...")
        face_service.load_face_map(face_map)
        logger.info("FAISS (GPU) Index Loaded.")

        time.sleep(2)

        # 3. Load InsightFace (GPU) SECOND
        # This will re-use the existing GPU context
        logger.info("Loading InsightFace (GPU) Model...")
        face_service.load_model()
        logger.info("InsightFace (GPU) Model Loaded.")
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    uvicorn.run(app, host="0.0.0.0", port=8001, ssl_keyfile="key.pem", ssl_certfile="cert.pem",log_level="info")
